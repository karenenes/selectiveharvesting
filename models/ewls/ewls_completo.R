    ...
    public = list(
        ...
        fit = function(x, y){  
          ...
          if (length(levels(y.factor)) > 1 && min(table(y.factor))>2) {
              ...
              #x = normalize(x, method = "range", range = c(0,1), margin = 1, on.constant = "quiet")
              #x = normalize(x, method = "standardize", margin = 1, on.constant = "quiet")
      ...
      predict = function(x)
      {
        ...
        ylabel = c(rep(0, ncol(x)))
        ylabel[match(as.numeric(targets), as.numeric(colnames(x)))] <- 1
