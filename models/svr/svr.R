...
SVR <- R6Class("SVR",
    ...
    public = list(
        ...
        getName = function() 
        {
          name <- sprintf("Support Vector Machine")
          return(name)
        ...
        fit = function(x, y){  
          ...
          if (length(levels(y.factor)) > 1 && min(table(y.factor))>2) {
              ...
              if (length(levels(y.factor[idx_training])) > 1 && min(table(y.factor[idx_training]))>2) {
                ...
                private$model <- svm(x = x_t, y = y_t, type = "eps-regression", gamma = as.numeric(fit_1$best.parameters), probability = TRUE)
               
                pred_treino <- predict(private$model, x_t, probability = TRUE)
                weights_2 = as.vector((y_t - pred_treino)^2)
                ...
                if(i < 10000){
                  ...
                  private$model_2 <- svm(x = x_t2, y = y_t2, type = "eps-regression", gamma = as.numeric(fit_2$best.parameters), probability = TRUE)

                  p_val1 <- predict(private$model, x_val, probability = TRUE)
                  erro_val1 = as.vector((y_val - p_val1)^2)

                  p_val2 <- predict(private$model_2, x_val, probability = TRUE)
        ...
        predict = function(x)
        {
          ...
          yhat <- predict(private$model, x, probability = TRUE)
          ...
          if(sum(!is.na(private$gray_interval))!=0){
            ...
            if(any(gray_inds)) {
              ...
              yhat_2 <- predict(private$model_2, x_test, probability = TRUE)
