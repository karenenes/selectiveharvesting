...
main <- function(filenames, output_path)
{
  ...
  for (filename in filenames) {
    ...
    # Checks number of valid tests.
    if (results$valid_tests != results$ntests_requested) {
      cat(sprintf("\n\nWARNING!\nIn file: %s\n", filename))
      cat(sprintf("%d valid tests out of ", results$valid_tests))
      cat(sprintf("%d intended!\n", results$ntests_requested))
    }
    ...
    final_index <- -1 + initial_index + length(results$runs)
    for (aggregated_index in initial_index:final_index) {
      # Appends the results.
      aggregated[[aggregated_index]] <- results$runs[[run_index]]
      # Appends the seeds.
      seeds[[aggregated_index]] <- results$runs[[run_index]]$seeds
      # Increments total run time.
      initial_run_time <- results$runs[[run_index]]$initial_time
      final_run_time   <- results$runs[[run_index]]$final_time
...
saveResults <- function(output_path, tests, settings, type)
{
  ...
  for(i in 1:length(tests)) {
    ...
    if (length(list_obs)) rownames(list_obs[[i]]) <- list_recruited[[i]]
  }
  ...
  # Saves the observed design matrix if requested.
  if (!is.null(settings$save_observations) && settings$save_observations)
...
{
  stopifnot(settings1$action_selector$getName() == settings2$action_selector$getName())
  stopifnot(settings1$score_mapper$getName() == settings2$score_mapper$getName())
  compareFeatureSets(settings1$feature_set, settings2$feature_set)
  ...
  stopifnot(settings1$save_observations == settings2$save_observations)
  ...
  stopifnot(all(unlist(Map(function(i) settings1$models[[i]]$getName() == settings2$models[[i]]$getName(),
                           1:max(settings1$nmodels, settings2$nmodels)))))
}

##
 # Compares two feature sets.
 ##
compareFeatureSets <- function(fs1, fs2)
{
  if (is.null(fs1) && is.null(fs2)) return()
  stopifnot(fs1$include_structural == fs2$include_structural)
  stopifnot(fs1$include_triangles == fs2$include_triangles)
  stopifnot(fs1$include_most == fs2$include_most)
  stopifnot(fs1$include_rw == fs2$include_rw)
  stopifnot(fs1$include_counts == fs2$include_counts)
  stopifnot(fs1$include_fractions == fs2$include_fractions)
  stopifnot(fs1$include_attrib_fractions == fs2$include_attrib_fractions)
}
